...
<div class="main-content">
    ...
    <div id="page-wrapper">
        <div class="main-page">
            <div class="tables">
                <h2 class="title1">班级列表</h2>
                <div class="table-responsive bs-example widget-shadow">
                    <h4>所有班级列表:</h4>
                    <div>
                        <form class="form-inline" method="get"
                              action="${pageContext.request.contextPath}/class/getClass">
                            <div class="form-group">
                                <input type="text" name="name" class="form-control" placeholder="班级名称" value="${sel_classes.name}">
                            </div>

                            <div class="form-group">
                                <input type="date" name="openDate" class="form-control"
                                       value="<fmt:formatDate value="${sel_classes.openDate}" type="Date"
                    ...
                    <table class="table table-bordered">
                        ...
                        <c:forEach items="${pageInfo.list}" var="classes" varStatus="count">
                            <tr>
                                <th scope="row">${count.count}</th>
                                <td>${classes.id}</td>
                                <td>${classes.name}</td>
                                <td>
                                    <fmt:formatDate value="${classes.openDate}" type="Date"
                                                    pattern="yyyy-MM-dd"></fmt:formatDate>
                                </td>
                                <td>
                                    <fmt:formatDate value="${classes.endDate}" type="Date"
                                                    pattern="yyyy-MM-dd"></fmt:formatDate>
                                </td>
                                <td>
                                    <a href="${pageContext.request.contextPath}/class/findById/${classes.id}">编辑</a>
                                    <a href="${pageContext.request.contextPath}/class/remove?id=${classes.id}">删除</a>
                                </td>
                            </tr>
                        </c:forEach>
                    </table>
                    <div style="float: right; font-size: 1.5rem;">
                        <nav>
                            <ul class="pagination">
                                <li style="margin-left: 0.9375rem;">
                                    <c:if test="${pageInfo.pageNum>1}">
                                        <a href="${pageContext.request.contextPath}/class/getClass?start=${pageInfo.prePage}&id=${sel_classes.id}&openDate=<fmt:formatDate value="${sel_classes.openDate}" pattern="yyyy-MM-dd" type="Date"></fmt:formatDate>"
                                ...
                                <li>
                                    <c:if test="${pageInfo.pageNum<pageInfo.pages}">
                                        <a href="${pageContext.request.contextPath}/class/getClass?start=${pageInfo.nextPage}&id=${sel_classes.id}&openDate=<fmt:formatDate value="${sel_classes.openDate}" pattern="yyyy-MM-dd" type="Date"></fmt:formatDate>"
